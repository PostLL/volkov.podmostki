...
require __DIR__ . '/api/robokassa-config.php';

$invId = (string) ($_REQUEST['InvId'] ?? $_REQUEST['InvoiceID'] ?? $_REQUEST['inv_id'] ?? '');
$codes = [];
$appBaseUrl = robokassa_get_app_base_url();
...
<div class="player">
            ...
            <?php foreach ($codes as $idx => $item): ?>
                <article class="ticket-card" data-code="<?= htmlspecialchars($item['code'], ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <div class="ticket-actions">
                        <a class="entry-action ticket-btn ticket-btn--use" href="<?= htmlspecialchars($appBaseUrl, ENT_QUOTES, 'UTF-8') ?>/?code=<?= urlencode($item['code']) ?>">Использовать</a>
                        <button class="entry-action ticket-btn ticket-btn--share ticket-share-btn" type="button" data-code="<?= htmlspecialchars($item['code'], ENT_QUOTES, 'UTF-8') ?>">Поделиться</button>
                    </div>
                </article>
            <?php endforeach; ?>
        <?php endif; ?>
    ...
    <section class="notice-card">
        <h2>Важно</h2>
        <p>Сделайте скриншот данной страницы или отправьте код себе в мессенджер, чтобы не потерять.</p>
        <p><strong>Техподдержка:</strong> support@podmostki.ru · +7 (900) 000-00-00</p>
    </section>
</div>

<script>window.PodmostkiAppBaseUrl = <?= json_encode($appBaseUrl, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;</script>
